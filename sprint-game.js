...
(function () {
  ...
  async function bumpLeaderboard(o) {
    ...
    await pingActivity(o, gBest);
    return { globalBest: gBest, title: titleForScore(gBest) };
  }

  // ---- daily activity ping: one row per candidate per calendar day, session count + best-so-far ----
  // activity/daily/<YYYY-MM-DD>/<ek> = { name, email, classCode, plays, bestScore, updatedAt }
  function dateKey(d) { d = d || new Date(); return d.toISOString().slice(0, 10); }
  async function pingActivity(o, bestScore) {
    try {
      var ek = emailKey(o.email), day = dateKey(), path = "activity/daily/" + day + "/" + ek;
      var cur = obj(await rest.get(path));
      await rest.put(path, {
        name: o.name, email: o.email, classCode: o.classCode || null,
        plays: (cur.plays || 0) + 1, bestScore: Math.max(cur.bestScore || 0, bestScore || o.score || 0),
        updatedAt: Date.now()
      });
    } catch (e) { /* non-critical */ }
  }
  // last N days of activity, each day = { date, rows:[...] }
  async function recentActivity(days) {
    days = days || 14;
    var out = [], d = new Date();
    for (var i = 0; i < days; i++) {
      var day = dateKey(d);
      var rows = list(await rest.get("activity/daily/" + day));
      out.unshift({ date: day, rows: rows });
  ...
  window.SprintGame = {
    ...
    recentActivity: recentActivity, dateKey: dateKey
  };
})();
